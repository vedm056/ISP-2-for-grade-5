...
var container = document.getElementById('container');
                  ...
                  container.addEventListener('mousemove', function(event) {
                    ...
                    mouseX += 500;
                    mouseY += 500;
                    // Calculate the rotation angles based on mouse coordinates
                    var rotationY = (mouseY - (container.offsetHeight)) / 9;
                    var rotationX = (mouseX - (container.offsetWidth)) / 9;
              
                    // Apply the rotation to the text element
                    text.style.transform = 'rotateX(' + rotationX + 'deg) rotateY(' + rotationY + 'deg)';
                  });
...
var container2 = document.getElementById('container2');
    ...
    container2.addEventListener('mousemove', function(event) {
      ...
      text2.style.left = mouseX2 + 'px';
      text2.style.top = mouseY2 + 'px';
    });
    container2.addEventListener('mousemove', function(event) {
      var mouseX2 = event.clientX;
      var mouseY2 = event.clientY;

      // Calculate the angle between the text and the mouse pointer
      var angle = Math.atan2(mouseY2 - (container.offsetHeight / 2), mouseX2 - (container.offsetWidth / 2));
      
      // Convert the angle to degrees and rotate the text element
      text2.style.transform = 'rotate(' + (angle * (180 / Math.PI)) + 'deg)';
    });